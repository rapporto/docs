...
Доступ к библиотеке осуществляется через репозиторий Nexus, подключение к которому настраивается в файле ``settings.gradle.kts``. В полях ``username`` и ``password`` необходимо указать значения, предоставленные :ref:`Службой технической поддержки <support>`:
...
Далее необходимо подключить библиотеку к проекту. Для этого в файл ``app/build.gradle.kts`` добавляется соответствующая зависимость. Вместо ``lib_version`` следует указать актуальную версию библиотеки, информацию о которой также предоставит служба технической поддержки:
