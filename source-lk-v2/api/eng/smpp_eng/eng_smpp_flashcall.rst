...
To send a FlashingCall type message to the Partner, you need to :ref:`establish a connection <linkSettingeng>` to the server and transmit the ``submit_sm`` packet to the Service Provider.  
...
To send a VoiceCode, you must set the message type as ``FlashingCall`` and submit a request containing the code.
...
|                           |                          | | Maximum FlashingCall message length: 2000 characters.                                                    | 
...
|                           |                          |                                                                                                            |
|                           |                          | | The text for *FlashingCall* messages must contain a 4-digit code, which is used for the call to the      | 
|                           |                          |   subscriber. Otherwise, the message will be rejected with the error “Flashing Call text should            |
|                           |                          |   contain a 4-digit code”. To send *Voice Code*, you must specify the message type as ``FlashingCall``     |
|                           |                          |   and submit a request containing the code.                                                                |
|                           |                          | | The specified code (without any accompanying text) will be transmitted to the operator, who will insert  |
|                           |                          |   it into the message template.                                                                            |
+---------------------------+--------------------------+------------------------------------------------------------------------------------------------------------+
...
.. note:: For TLV parameters related to cascading message sending, please refer to the corresponding section on the website.
...
|                     | “Detect automatically” option (default value) option| the conditions for the occurrence of this error.        |
