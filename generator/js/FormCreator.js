...
function addEvents() {
    ...
    $('#options-title').on('click', (e) => {
        ...
        $('#messages-content').classList.add('hidden');
        $('#options-title').classList.add("div-title-active");
        $('#messages-title').classList.remove("div-title-active");

    });

    $('#messages-title').on('click', (e) => {

        $('#messages-content').classList.remove('hidden');
        $('#options-content').classList.add('hidden');
        $('#options-title').classList.remove("div-title-active");
        $('#messages-title').classList.add("div-title-active");
...
function addWeekDayCheckbox(parent, caption, id, checked) {
    ...
    let label = parent.add('label', {
        innerHTML: caption,
        className: 'week-label',
        onclick: (e) => {
            if (document.getElementById(id).checked) label.classList.add('week-label-checked');
            else label.classList.remove('week-label-checked');
        }
    });

    if (checked) label.classList.add('week-label-checked');
...
function addMessage(order) {
    ...
    addMRow(mt, 'Надпись на кнопке').add('input', {
        id: `viber-caption-url-${order}`,
        type: 'text'
    });
    addMRow(mt, 'Ссылка для кнопки').add('input', {
        id: `viber-action-url-${order}`,
        type: 'text'
    });

    addMRow(mt, 'Ссылка на изображение').add('input', {
        id: `whatsApp-image-url-${order}`,
        type: 'text'
    });

    addMRow(mt, 'Ссылка на изображение').add('input', {
        id: `push-image-url-${order}`,
        type: 'text'
    });
    addMRow(mt, 'Ссылка на HTML-страницу').add('input', {
        id: `push-html-url-${order}`,
        type: 'text'
    });
    addMRow(mt, 'Заголовок сообщения').add('input', {
        id: `push-title-${order}`,
        type: 'text'
    });
    addMRow(mt, 'Идентификатор абонента').add('input', {
        id: `push-externalUserId-${order}`,
        type: 'text'
    });
    ...
    addMRow(mt, 'Данные для передачи в приложение (JSON)').add('input', {
        id: `push-customPayload-${order}`,
        type: 'text'
    });
    addMRow(mt, 'Дополнительные данные').add('input', {
        id: `push-callbackData-${order}`,
        type: 'text'
    });
