HTTP API
=============

:abbr:`HTTP (HyperText Transfer Protocol)` :abbr:`API (Application Programming Interface)` v.5.0 s designed to be integrated into the Partner's system in order to send certain types of messages and receive the statuses of sent messages.

The integration provides the ability to send both incoming traffic (Mobile Terminated message) and outgoing traffic (Mobile Originated message). 


Types of messages
-------------------

When integrating via HTTP API, the transmission of the following types of messages is supported:

- :term:`FlashingCall` (:term:`VoiceCode`);
- Push;
- SMS;
- Viber;
- VK;
- WhatsApp.



.. toctree::
   :maxdepth: 1
   :hidden:

   eng_http_auth
   eng_http_request
   eng_http_sms
   eng_http_viber
   eng_http_whatsapp
   eng_http_vk
   eng_http_push
   eng_http_flashcall
   eng_http_cascade
   eng_http_mo
   eng_http_status
   eng_http_short_link