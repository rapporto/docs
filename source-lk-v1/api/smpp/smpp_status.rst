...
.. important:: 1. Для SMS-сообщений, отправленных абонентам оператора Мегафон, с 01.03.2023 прекращена передача статусов ``DELIVRD(2)`` и ``UNDELIV(5)``.
    2. При отправке абоненту в мессенджер Telegram авторизационных кодов возможно получение только статусов ``DELIVRD(2)`` и ``UNDELIV(5)``. 
