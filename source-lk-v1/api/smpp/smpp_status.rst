...
.. warning:: Внимание! Для SMS-сообщений, отправленных абонентам оператора Мегафон, с 01.03.2023 прекращена передача статусов ``DELIVRD`` и ``UNDELIV``.
...
+===========================+==========================+============================================================================================================+
| source_addr               | string                   | Сервисное имя, с которого отправлено сообщение.                                                            |
+---------------------------+--------------------------+------------------------------------------------------------------------------------------------------------+
| destination_addr          | string                   | Телефон абонента, которому отправлено сообщение.                                                           |
+---------------------------+--------------------------+------------------------------------------------------------------------------------------------------------+
...
..
  ...
  ----------------------------
  SMPP-интеграция в отчёте о доставке передаёт один из следующих статусов:
  - ``ENROUTE`` — сообщение отправлено;
  - ``DELIVRD`` — сообщение успешно доставлено;
  - ``REJECTD`` — сообщение отклонено на этапе отправки (текст сообщения не прошел модерацию);
  - ``EXPIRED`` — сообщение удалено из очереди по истечении времени жизни;
  - ``UNDELIV`` — все остальные случаи недоставки;
  - ``UNKNOWN`` — платформа Сервис-провайдера в течение установленного времени не получила ответа от оператора.
...
.. tabs::
    ...
    .. tab:: Push
      ...
      |         | :abbr:`HMS (Huawei Mobile Services)`, RuStore) , через которого выполняется отправка уведомления, вернул ошибку.                               |
      +---------+------------------------------------------------------------------------------------------------------------------------------------------------+
