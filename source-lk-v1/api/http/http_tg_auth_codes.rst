...
- каскадную передачу сообщений с кодами в SMS в случае недоставки в Telegram (при необходимости).
...
:term:`Каскадная рассылка` сообщений по умолчанию недоступна. Для подключения и настройки Партнёру необходимо обратиться в :ref:`службу технической поддержки <support>` Сервис-провайдера.

Для передачи кодов в каскаде ``Telegram`` > ``SMS`` Партнёру необходимо отправлять запрос на отправку только первого сообщения каскада. 

.. important::
    
    При этом указать тип сообщения только ``SMS``.

В процессе обработки сообщение будет конвертировано в TG-формат и передано в Telegram. 

Далее, в случае недоставки в Telegram, каскад сработает автоматически на основе настроек, и код будет доставлен абоненту в SMS.
