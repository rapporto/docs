...
1. если использовать правильные параметры (order_list = "F,S", f_resendCond = N, s_resendCond = Y), 
то будет отправлено только первое сообщение каскада (Flashing Call). 
Т.к. срабатывание каскада ПОСЛЕ сообщения типа Flashing Call запрещено (f_resendCond = N), то каскадное СМС не отправится. 
Параметр s_resendCond = Y при этом не имеет значения, т.к. это настройка срабатывания каскада ПОСЛЕ СМС.
 - будет отправлено только первое сообщение из каскада (Whatsapp).
