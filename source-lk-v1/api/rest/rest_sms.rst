...
.. csv-table:: 
    ...
    "**message/data/serviceNumber**", "string", "Сервисное имя, от которого осуществляется отправка сообщения. Для передачи в запросе параметр является обязательным, если сервисное имя не указано в настройках клиента. Если указано, то в запросе становится необязательным. Если указано в настройках клиента и передано в запросе, то сообщение абоненту будет отправлено от имени, указанного в запросе."
