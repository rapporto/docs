...
.. csv-table:: 
    ...
    "**message/data/serviceNumber**", "string", "Сервисное имя, от которого осуществляется отправка сообщения. Для передачи в запросе параметр является обязательным, если сервисное имя не указано в настройках клиента. Если указано, то в запросе становится необязательным. *Примечание.* Если в запросе не будет передано имя, то подтянется автоматом имя, указанное в настройках клиента. Если имя будет передано в запросе, то сообщение будет отправлено от имени, которое указано в запросе, а имя прописанное в настройках клиента, будет проигнорировано."
