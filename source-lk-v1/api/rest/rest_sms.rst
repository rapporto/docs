...
.. csv-table:: 
    ...
    "**message/data/serviceNumber**", "string", "Сервисное имя, от которого осуществляется отправка сообщения. Для передачи в запросе параметр является обязательным, если сервисное имя не указано в настройках клиента. Если указано, то в запросе становится необязательным. При отправке сообщения абоненту приоритетным является имя, указанное в запросе."
