...
Причиной для переотправки сообщения является неполучение одного из статусов ``DELIVERED`` ("Получено") или ``READ`` ("Прочитано") параметра ``state`` в течение времени жизни сообщения (параметр ``ttl``). 
